...
use crate::authentication::check::{check_gas_request, check_gas_response, TokenType};
...
impl GASPackageResponse {
    ...
    pub fn print_gas(&self) {
        for i in 0..self.n_sas {
            ...
            let nonce =
                u32::from_be_bytes(self.raw[(16 + 80 * i)..(20 + 80 * i)].try_into().unwrap());
            let token = str::from_utf8(&self.raw[(20 + 80 * i)..(80 + 80 * i)]).unwrap();

            print!("{id}:{nonce}:{token}+");
        }

        let token = str::from_utf8(&self.raw[(20 + 80 * self.n_sas)..]).unwrap();
        println!("{token}");
    }
}
